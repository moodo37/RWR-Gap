...
"""
Train and evaluate a ResNet-152 classifier on the ISIC RWR-Gap benchmark.

This script:
  - Trains a binary classifier (normal vs. lesion/marker)
...
# ---------------------------------------------------------------------
# Model factory: ResNet-152
# ---------------------------------------------------------------------
def build_resnet152(num_classes: int = 2, weights: str = "imagenet"):
    """
    Build a ResNet-152 classifier.

    Args:
        num_classes: Number of output classes (2 for binary classification).
        weights:
            - "imagenet": ImageNet-1K pretrained weights
    ...
    if weights == "imagenet":
        ...
        raise ValueError(f"Invalid weights: {weights} (use 'scratch' or 'imagenet').")
...
def evaluate_classifier(model, loader, device):
    """
    Evaluate a classifier and return accuracy, AUC, and predictions.
    """
    model.eval()
    all_labels = []
    all_probs = []
...
def compute_rwr_gap(acc_all: float, acc_robust: float, acc_conflict: float):
    """
    Compute RWR-Gap using the definition in the paper.

    We define:
        Gap_robust   = Acc_all - Acc_robust
        Gap_conflict = 1 - Acc_conflict
        RWR-Gap      = Gap_robust + Gap_conflict
    """
    gap_robust = acc_all - acc_robust
...
def main(args):
    ...
    print(f"[Info] ResNet-152 weights: {args.weights}")

    # Output directory: out_dir / exp_name
    if args.exp_name:
    ...
    model = build_resnet152(num_classes=2, weights=args.weights).to(device)
    ...
    # -------------------------
    # Load best checkpoint and evaluate on all test splits
    # -------------------------
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="RWR-Gap Training & Evaluation (ISIC, ResNet-152)"
    )
    ...
    parser.add_argument(
        ...
        help="Root directory of the ISIC RWR-Gap dataset.",
    )
    parser.add_argument(
        "--out_dir",
        type=str,
        default="outputs/isic",
    ...
    parser.add_argument(
        ...
        help="Initialization for ResNet-152.",
    )
